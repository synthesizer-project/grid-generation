...
def create_cloudy_input(
    ...
    # Resolve depletion from config: None/"None"/falsy -> no depletion
    depletion_model = parameters.get("depletion_model")
    if isinstance(depletion_model, str) and depletion_model.lower() in (
        "none",
        "null",
        "",
    ):
    ...
    abundances = Abundances(
        ...
        depletion_scale=parameters.get("depletion_scale")
        if depletion_model
        else None,
    )
    ...
    # 	or cloudy_version.startswith("c25"):
    #     cloudy = cloudy25
    # else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--by-photoionisation-grid-point", action="store_true", default=False
    )
    ...
    if extra_cloudy_paramfile:
        ...
        extra_cloudy_paramfile_name = extra_cloudy_paramfile.split("/")[-1]
    ...
    (
        ...
        incident_axes, incident_axes_values, verbose=True
    )
    ...
    if len(variable_photoionisation_params) > 0:
        ...
        (
            ...
            photoionisation_axes, variable_photoionisation_params, verbose=True
        )
    ...
    for incident_index, (
        incident_params_tuple,
        incident_index_tuple,
    ) in enumerate(zip(incident_model_list, incident_index_list)):
        ...
        np.save(
            f"{output_directory}/{incident_index}/input", np.array([lam, lnu])
        )
        ...
        shutil.copyfile(
            "linelist.dat", f"{output_directory}/{incident_index}/linelist.dat"
        )
